...
function updateDashboardTotals(ads) {
  ...
  if (document.getElementById('earnings')) {
    document.getElementById('availableBalance').textContent = `$${totalEarnings.toFixed(2)}`;
  }
...
function initializeChart(ads) {
  ...
  // Group data by day
  const groupedData = groupDataByDay(ads);
...
function groupDataByDay(ads) {
  ...
  if (!ads || ads.length === 0) {
    // If no data, just return empty labels for current week
  ...
  ads.forEach(ad => {
    ...
    const dayIndex = dateRange.indexOf(adDate);
...
function updateBalanceUI(balanceData) {
  ...
  if (available >= 100) {
    ...
    withdrawMessage.textContent = `Minimum withdrawal amount is $100`;
  ...
  paymentsTable.innerHTML = paymentsArray.map(payment => {
    ...
    return `
      <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
        ...
        <td class="py-3 px-4 font-medium">$${totalEarnings.toFixed(2)}</td>
